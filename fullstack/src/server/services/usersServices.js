            ...
            bcrypt.hash(newUser.password, salt).then((hash) => {
              ...
              newUser
                ...
                .catch((err) => {
                  ...
                  return res
                    .status(400)
                    .json({ error: "Can't register the user. Please contact website admin" });
                });
            });
          })
          .catch((err) => {
    ...
    if (req.isAuthenticated()) {
      ...
      if (!id) {
        User.deleteOne({ _id: id }, (err) => {
          ...
          return res
            .status(400)
            .json({ error: "Can't delete the user. Please contact website admin" });
        });
      } else {
        return res
          .status(400)
          .json({ error: "Can't delete the user. Check the values. Please contact website admin" });
      }
    } else {
